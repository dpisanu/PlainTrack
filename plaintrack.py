...
class TimeTrackerReport:
    ...
    def validate_configs(self):
        ...
        # NEUE REGEL: Sonderurlaub darf >= 0 sein
    ...
    def validate_day_files(self):
        ...
        try:
            ...
            self.errors.append("Dateinamen-Format ungültig (erwarte 1.txt, 2.txt...)")
            self.check_and_exit_if_failed("Dateinamen-Check")
            return
        ...
        for filename in files:
            ...
            if not file_errors:
                self.log_step(f"Tag {day_num.ljust(2)} : OK")
            else:
                err_msg = ", ".join(file_errors)
                self.log_step(f"Tag {day_num.ljust(2)} : FEHLER ({err_msg})", False)
                self.errors.append(f"Tag {day_num}: {err_msg}")
